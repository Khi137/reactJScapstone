...
export default function MovieList() {
  ...
  let fetchApi = async () => {
    try {
      let result = await fetchApiMovie("getListMovies");
       result = result.filter(movie => (movie.dangChieu && movie.sapChieu ) ||( movie.sapChieu && movie.dangChieu==false));
      // console.log(result)
      setListMovie(result);
    } catch (err) {}
  };
  ...
  const settings = {
    ...
    dots: true,
    speed: 500,
    slidesToShow: Math.min(listMovie.length, 5),
    slidesToScroll: Math.min(listMovie.length, 5),
    initialSlide: 0,
    arrows: false,
    responsive: [
      {
        breakpoint: 1279,
        settings: {
          slidesToShow: Math.min(listMovie.length,4),
          slidesToScroll: Math.min(listMovie.length, 4),
          dots: false,
          arrows: false,
          speed: 500,
        },
      },
      {
        ...
        settings: {
          dots: false,
          slidesToShow: Math.min(listMovie.length, 3),
          slidesToScroll: Math.min(listMovie.length, 3),
          initialSlide: 0,
          arrows: false,
          speed: 500,
        },
  ...
  return (
    ...
    >
      ...
      <div className="relative xl:h-[32rem] md:h-[31rem] h-[20rem]  flex items-center justify-center container mx-auto">
        ...
        <button
          onClick={previous}
          className="text-white-300 xl:text-5xl md:5xl text-3xl absolute xl:left-10 md:left-5 left-2 xl:top-2/5 md:top-1/3 top-1/3"
        >
          <CaretLeftOutlined />
        </button>
        {/* // NEXT  */}
        <button
          onClick={next}
          className="text-white-300 xl:text-5xl md:5xl text-3xl absolute xl:right-10 md:right-5 right-2 xl:top-2/5 md:top-1/3 top-1/3"
        >
         <CaretRightOutlined />
        </button>
